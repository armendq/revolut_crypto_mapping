...
# Revolut universe (provided by user – keep full, we'll filter by Binance availability)
REVOLUT_TICKERS = {
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if r.status_code == 200:
                return r
            # 429/418 rate limits: brief sleep then retry
...
def analyze_symbol(symbol: str, btc_1h_close: List[float]) -> Optional[dict]:
    ...
    # Determine signals
    # Confirmed breakout if close pierces above HH20 + buffer * ATR with volume thrust and lower TF confirm.
...
def main() -> None:
    ...
    # Universe construction: Revolut tickers mapped to Binance USDT symbols, filtered by availability
    exch = fetch_exchange_usdt_symbols()
    if not exch:
        # Fallback: empty exchange info – produce a safe HOLD summary
        payload = {
        ...
        print("exchangeInfo unavailable – wrote HOLD summary.")
        return

    # Build list of eligible symbols from Revolut set -> append USDT, skip stables
    binance_symbols_available = set(exch.keys())
    tickers = [t for t in REVOLUT_TICKERS.keys() if t not in STABLES]
    mapping: Dict[str, str] = {}  # Revolut ticker -> Binance symbol string
    ...
    for t in tickers:
        ...
        if sym in binance_symbols_available:
            ...
            # Some tokens have different symbols on Binance; you can extend exceptions here if needed.
            missing_binance.append(t)

    # Pre-fetch BTC 1h as RS baseline
    btc_symbol = "BTCUSDT"
    ...
    for t, sym in mapping.items():
        ...
        # brief pacing to avoid hammering
        time.sleep(0.05)
        if not info:
            no_data.append(t)
            continue

        # attach Revolut ticker & rotation/immutable annotations
        info["ticker"] = t
    ...
    # If we have confirmed breakouts, those take precedence
    signal_type = "HOLD"
    orders: List[dict] = []
    if confirmed:
        signal_type = "B"
        # Build minimal orders payload; position sizing handled downstream
        for o in confirmed:
            orders.append({
                "ticker": o["ticker"],
                "symbol": o["symbol"],
    ...
    # Build candidates payload (compact but actionable)
    cand_payload = [{
        "ticker": c["ticker"],
        "symbol": c["symbol"],
        "last": round(c["last"], 8),
