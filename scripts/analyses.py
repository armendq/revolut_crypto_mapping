...
# Universe admission: include all unless liquidity is extremely low
UNIVERSE_VOL_MIN_USD = 500_000  # skip only if 24h volume known and < $500k

# Breakout trigger (no upper cap)
BR_MIN = 0.008          # +0.8% vs previous close
RVOL_MIN = 1.5          # relaxed to catch early moves
HH_CONFIRM = 0.001      # last close >= 15-bar high * (1 + HH_CONFIRM)

# Micro-pullback (not mandatory; we still trade without it)
FADE_MAX = 0.015        # 1.5%
RANGE_MAX = 0.020       # 2.0%
...
STRONG_ALLOC = 0.60
WEAK_ALLOC = 0.30
LANE_B_CAP_FACTOR = 0.50  # CG-only lane sized at 50% of normal per-trade cap
...
# Default spread used when order book not available
...
def check_regime() -> Dict[str, Any]:
    ...
    ok = (last > vw) or (last > e9)  # relaxed: VWAP OR EMA9
...
def fetch_meta_for_symbol(ticker: str, binance_sym: Optional[str]) -> Dict[str, Any]:
    ...
    # 3) CoinGecko only (price/vol + momentum)
    if cid:
...
def aggressive_breakout(bars_1m: List[Dict[str, float]]) -> Optional[Dict[str, float]]:
    ...
    if pct < BR_MIN:  # no upper cap
        return None

    # RVOL vs last 15 bars (if volume missing -> assume 2.0 to allow CG lane)
    ...
    # 15m momentum proxy: EMA9 slope up on 1m closes
    closes = [b["c"] for b in bars_1m[-90:]]  # ~last 90 min points if available
...
def micro_pullback_ok(bars_1m: List[Dict[str, float]]) -> Optional[Dict[str, float]]:
    ...
    return None  # optional; caller will still use breakout bar low as stop
...
def main():
    ...
    for m in mapping:
        ...
        if not tkr:
            continue
        if tkr in ("ETH", "DOT"):  # ETH held, DOT staked/untouchable
            continue
        sym = best_binance_symbol(m)
        meta = fetch_meta_for_symbol(tkr, sym)
        # Skip only if we know vol_usd and it's extremely low
        vol_ok = True
    ...
    # 3) Signals: breakout + RVOL + confirmation, optional micro pullback, trend up
    ...
    for u in universe:
        ...
        # Score: pct * (1 + rvol) * (1 + 1h momentum if known)
    ...
    for c in chosen:
        ...
        # enrich snapshot candidate for downstream consumers
        c["t1"] = t1
        c["t2"] = t2
        c["usd"] = usd
